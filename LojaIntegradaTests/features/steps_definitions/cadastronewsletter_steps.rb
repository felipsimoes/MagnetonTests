...
Então(/^eu visualizo a mensagem de confirmação$/) do
  expect(@home.newsletter.confirmation)
    .to have_content('Obrigado por se inscrever! '\
          'Aguarde novidades da nossa loja em breve')
end

Então(/^eu visualizo a mensagem de aviso$/) do
  expect(@home.newsletter.alert)
    .to have_content('Seu e-mail já consta em nossa newsletter.')
end
