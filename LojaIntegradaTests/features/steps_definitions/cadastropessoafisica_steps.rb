...
Então(/^eu visualizo a confirmação de cadastro$/) do
  expect(@cadastro.criado_sucesso.text)
    .to have_content('Cliente criado com sucesso.')
end

Então(/^eu visualizo um alerta de email já cadastrado$/) do
  expect(@login.alerta.text)
    .to have_content('Já existe um cliente cadastrado com este email.')
end

Então(/^eu visualizo o alerta de tamanho de senha$/) do
  expect(@cadastro.validacao_senha.text)
    .to have_content('Certifique-se de que o valor tenha no mínimo '\
        '5 caracteres (ele possui 4).')
end
